...
from core.game1 import CBGame
# from utils.utils import safe_entropy
...
class CBSystemPlanner(DialogPlanner):
    def __init__(
        ...
        The following is a new conversation between Buyer and a Seller.
        {self.process_exp()}
...
class CBChatSystemPlanner(CBSystemPlanner):
    ...
    def heuristic(self, state: DialogSession) -> float:
        ...
        if not self.zero_shot:
            user_task_prompt = f"""
            Given a conversation between a Buyer and a Seller, please assess whether they have reached a deal after the conversation.
            You can choose amongst the following actions during a conversation to respond to the Buyer:
            {" ".join([f"[{da}]" for da in self.user_dialog_acts])}
            The following is a example conversation between a Buyer and a Seller.
            """.replace(
                "\t", ""
            ).strip()
            user_new_task_prompt = "The following is a new conversation between a Buyer and a Seller (you)."
            ...
            messages.append(
                {
                    "role": "user",
                    "content": f"{CBGame.SYS}: Have we reached a deal?",
                }
            )
        else:
            print("\n=== Zero-shot模式下的交易判断 ===")
...
class BuyerChatModel(BuyerModel):
    def __init__(
        ...
        if self.zero_shot:
            ...
            self.task_prompt = """
            Now enter the role-playing mode. In the following conversation, you will play as a buyer in a price bargaining game.
            You are the buyer who is trying to negotiate a good price for the item you want to purchase.
            The following is an example conversation between a Buyer and a Seller.
            """.replace(
                "\t", ""
            ).strip()
    ...
    def get_utterance_batched(
        ...
        if self.zero_shot:
            ...
            messages = [
                ...
                *self.prompt_examples,
                {"role": "system", "content": self.new_task_prompt},
                {"role": "system", "content": f"CONTEXT: You are the buyer. You want to buy a {state.item_name}. Your budget is around {state.buyer_price}. The seller's description is: '{state.seller_item_description}'."}
            ]
        if len(state) == 0:
...
class SellerModel(DialogModel):
    def __init__(
        ...
        You are the seller who is trying to sell your item at a good price.
		...
		{self.process_exp()}
		The following is a new conversation between another Seller and Buyer.
		"""
        self.task_prompt = self.task_prompt.replace("\t", "").strip()
    ...
    def get_utterance_w_da(self, state: DialogSession, action=None, mode='train') -> Tuple[str, str]:
        ...
        # 仅根据方括号内容提取对话行为
        da = self._extract_da_from_response(user_resp)

        # 去掉开头的 [DA] 标签，保留纯文本回复
        clean_resp = user_resp.strip()
        if clean_resp.lower().startswith(f"[{da.lower()}]"):
            clean_resp = clean_resp[len(da)+2:].lstrip()

        return da, clean_resp
    
    def _extract_da_from_response(self, response: str) -> str:
        """Extract dialog act from seller response using keywords and patterns"""
        # 仅按方括号提取 DA
        match = re.search(r"\[(.*?)\]", response)
        if match:
            candidate = match.group(1).strip()
            # 与已知 dialog_acts 不区分大小写比对
            for da in self.dialog_acts:
                if candidate.lower() == da.lower():
                    return da

        # 若未匹配到合法标签，默认返回 no deal
    ...
    def get_utterance(self, state: DialogSession, action=None, mode='train') -> str:
        ...
        if self.zero_shot:
            ...
            messages = [
                ...
                *self.prompt_examples,
                {"role": "system", "content": self.new_task_prompt},
                {"role": "system", "content": f"CONTEXT: You are the seller. You are selling a {state.item_name} for {state.seller_price}. Your item description is: '{state.seller_item_description}'. The buyer's initial thought/description is: '{state.buyer_item_description}'."}
            ]
            messages += self.__proccess_chat_exp(
