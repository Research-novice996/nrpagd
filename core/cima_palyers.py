...
from core.helpers import DialogSession, CIMADialogSession
from core.gen_models import GenerationModel, DialogModel
from core.game1 import CIMAGame
# from mcts.utils.utils import safe_entropy
from abc import ABC, abstractmethod
from collections import Counter
# from utils import reward_dict
...
class CIMAChatSystemPlanner(CIMASystemPlanner):
    ...
    def _get_generated_da(self, data) -> list:
        # convert generated responses to DA
        action_list = [CIMAGame.S_Confirmation, CIMAGame.S_Correction, CIMAGame.S_Hint, CIMAGame.S_Others,
                       CIMAGame.S_Question]
        pred_da = []
        for resp in data:
            resp = resp["generated_text"].strip()
...
class TeacherChatModel(TeacherModel):
    ...
    def __proccess_chat_exp(self, exp: DialogSession, da_prompt: str = '', max_hist_num_turns: int = -1,
                            use_role: bool = True):
        if len(exp) == 0:
            return []
    ...
    def get_utterance_batched(
        ...
        if self.zero_shot:
            messages = [
                {"role": "system", "content": self.task_prompt},
                {"role": "user",
                 "content": "You are the teacher who is trying to teach the student to translate '%s' into Italian. Please reply with only one short and succinct sentence. Please do not tell the student the answer or ask the student about other exercises. %s Now ask me an exercise." % (
                 state.sentence, da_prompt)}
            ]
        else:
            messages = [
                {"role": "system", "content": self.task_prompt},
...
class StudentChatModel(StudentModel):
    ...
    def __proccess_chat_exp(self, exp: DialogSession, max_hist_num_turns: int = -1, use_da: bool = True,
                            use_role: bool = True):
        if len(exp) == 0:
            return []
    ...
    def get_utterance(self, state: DialogSession, action=None, mode='train') -> str:
        ...
        if self.zero_shot:
            messages = [
                {"role": "system", "content": self.task_prompt},
                {"role": "user",
                 "content": "You are the student who is trying to translate an English sentence into Italian. You don't know the translation of '%s' in Italian. Please reply with only one short and succinct sentence. Are you ready to play the game?" % (
                     state.sentence)},
