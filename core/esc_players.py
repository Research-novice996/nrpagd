...
from core.helpers import DialogSession, EmotionSupportDialogSession
...
class ESCSystemPlanner(DialogPlanner):
    ...
    def heuristic(self, state: DialogSession) -> float:
        ...
        score = []
        reward_mapping = {
            EmotionalSupportGame.U_FeelWorse: -1.0,
            EmotionalSupportGame.U_FeelTheSame: -0.5,
            EmotionalSupportGame.U_FeelBetter: 0.1,
            EmotionalSupportGame.U_Solved: 1.0,
        }
        
        for da in sampled_das:
            if da in reward_mapping:
                reward_val = reward_mapping[da]
                score.append(reward_val)
...
class ESCChatSystemPlanner(ESCSystemPlanner):
    ...
    def predict(self, state: DialogSession, policy=None, ent_bound=None, agent_state=None) -> "Tuple[np.ndarray, float]":
        # test k times and compute prob. See num_return_sequences in the API
        # the value would be our objective function
        if self.use_policy_prior and policy is not None and agent_state is not None:
        ...
        # 对于MCTS，我们需要返回一个价值估计
        # 这里返回一个固定的中性值，因为真正的价值评估在heuristic方法中进行
        v = 0.0
        return prob, v
    ...
    def heuristic(self, state: DialogSession) -> float:
        ...
        print(f"\n=== 开始Heuristic评估 ===")
        print(f"Zero-shot模式: {self.zero_shot}")
        print(f"评估参数: {self.eval_args}")
        ...
        for i, msg in enumerate(messages):
            print(f"  消息{i+1} ({msg['role']}): {msg['content'][:200]}...")
        ...
        #     "num_return_sequences": 10,
        # }
        print(f"调用GPT进行{self.eval_args.get('num_return_sequences', 1)}次评估...")
        data = self.generation_model.chat_generate(messages, **self.eval_args)
        print(f"GPT返回了{len(data)}个响应")
        ...
        for i, resp in enumerate(data):
            print(f"  响应{i+1}: {resp.get('generated_text', resp)}")
        
        print(f"解析后的对话行为: {sampled_das}")
        ...
        score = []
        reward_mapping = {
            EmotionalSupportGame.U_FeelWorse: -1.0,    # reward_dict['esc']['worse']
            EmotionalSupportGame.U_FeelTheSame: -0.5,  # reward_dict['esc']['same']
            EmotionalSupportGame.U_FeelBetter: 0.1,    # reward_dict['esc']['better']
            EmotionalSupportGame.U_Solved: 1.0,        # reward_dict['esc']['solved']
        }
        ...
        for da in sampled_das:
            if da in reward_mapping:
                reward_val = reward_mapping[da]
                score.append(reward_val)
                print(f"  {da} -> {reward_val}")
            else:
                print(f"  未知对话行为: {da}")
        
        v = 0.0 if len(score) == 0 else np.mean(score)
        print(f"最终奖励值: {v} (来自{len(score)}个有效评估)")
        print(f"=== Heuristic评估结束 ===\n")
...
class TherapistChatModel(TherapistModel):
    ...
    def get_utterance_w_da(self, state: DialogSession, action) -> Tuple[str, str]:
        user_resp = self.get_utterance(state, action, mode='train')
        # extract da
