...
from core.helpers import DialogSession, EmotionSupportDialogSession, CIMADialogSession, CBDialogSession
...
class EmotionalSupportGame(DialogGame):
	...
	def get_next_state(self, state:DialogSession, action, agent_state: list = None, mode: str = 'train') -> DialogSession:
		...
		# Handle agent_state being None (for MCTS compatibility)
		...
		# For MCTS compatibility: return only next_state when agent_state is None
...
class CBGame(DialogGame):
	...
	def __init__(self, system_agent:DialogModel, user_agent:DialogModel, 
              planner, zero_shot, max_conv_turns=15, success_base=0.1):
		super().__init__('esc', CBGame.SYS, system_agent, CBGame.USR, user_agent, planner, zero_shot, success_base)
		self.max_conv_turns = max_conv_turns
		return
	...
	def get_next_state(self, state:DialogSession, action, agent_state: list = None, mode: str = 'train'):
		next_state = state.copy()
...
class CIMAGame(DialogGame):
	...
	def __init__(self, system_agent:DialogModel, user_agent:DialogModel, 
              planner, zero_shot, max_conv_turns=15, success_base=0.1):
		super().__init__('cima', CIMAGame.SYS, system_agent, CIMAGame.USR, user_agent, planner, zero_shot, success_base)
		self.max_conv_turns = max_conv_turns
		return
		...
		return {
			"system": {
				"dialog_acts": [
					...
					CIMAGame.S_Question, CIMAGame.S_Others,
				],
			},
			"user": {
				"dialog_acts": [
	...
	def map_user_action(self, v, sampled_das):
		...
		da_dict = ddict(int)	
		for sample_da in sampled_das:
			if sample_da != CIMAGame.U_Correct:
				da_dict[sample_da] += 1
		if len(da_dict) == 0:
			return CIMAGame.U_DidNotTry
		max_freq_da = max(da_dict, key=lambda x: da_dict[x])
		return max_freq_da
