...
from core.game1 import DialogGame, CIMAGame
...
class NRPAPlanner:
    ...
    def _get_legacy_word_count_score_for_simulated_segment(self, simulated_turns,
        ...
        for turn_idx_in_sim_segment, turn in enumerate(simulated_turns):
            ...
            if len(turn) >= 3:
                speaker = turn[0]
                if speaker == CIMAGame.SYS:  # Check for Teacher
                    teacher_reply_content = turn[2]
                    if isinstance(teacher_reply_content, str):
                        word_count = len(teacher_reply_content.split())
                        # print("-------------------- LEGACY SCORE TIEBREAK CALCULATION (NRPA's First Simulated Teacher Utterance) --------------------")
                        ...
                        # print(f"[score_legacy_tiebreak] (NRPA Sim) 选择用于计分的 Teacher 在当前模拟段的第一条回复 (来自历史记录索引 {actual_history_idx}, 发言者 {speaker}): '{teacher_reply_content}'")
                        # print(f"[score_legacy_tiebreak] (NRPA Sim) 计算词数: {word_count}，legacy tiebreak 得分: {word_count}")
                        # print("-----------------------------------------------------------------------------------------------------------------------------")
                        return word_count
                    else:
                        # print(f"[score_legacy_tiebreak] (NRPA Sim) Teacher 在当前模拟段的第一条回复 (历史记录索引 {actual_history_idx}, 发言者 {speaker}) 的内容不是字符串 ('{teacher_reply_content}'), legacy tiebreak 得分为 0")
                        if resulting_state_full_history_for_debug:
                            # print(f"[score_legacy_tiebreak] (NRPA Sim) 当时完整对话历史 (共 {len(resulting_state_full_history_for_debug)} 轮):")
                            for i, h_turn in enumerate(resulting_state_full_history_for_debug):
                                # print(f"  轮次 {i}: {h_turn}")
        ...
        # print(f"[score_legacy_tiebreak] (NRPA Sim) 在当前模拟段 (从索引 {len_history_before_simulation_for_debug} 开始) 未找到 Teacher 的有效回复，legacy tiebreak 得分为 0")
        if resulting_state_full_history_for_debug:
            # print(f"[score_legacy_tiebreak] (NRPA Sim) 当时完整对话历史 (共 {len(resulting_state_full_history_for_debug)} 轮):")
            for i, h_turn in enumerate(resulting_state_full_history_for_debug):
                prefix = "  "
    ...
    def score(self, resulting_state: DialogSession, len_history_before_simulation: int):
        ...
        # 检查对话是否达到成功状态
        if hasattr(resulting_state, 'history') and resulting_state.history:
            last_turn = resulting_state.history[-1]
            if len(last_turn) >= 2 and last_turn[1] == CIMAGame.U_Correct:
                initial_score = 1.0
                print(f"[Scoring] 检测到 'Correct' 状态，设定基础分为 1.0")
            else:
                initial_score = 0.0
                print(f"[Scoring] 未检测到 'Correct' 状态，设定基础分为 0.0")
        else:
            initial_score = 0.0
            print(f"[Scoring] 对话历史为空，设定基础分为 0.0")

        # 计算轮次惩罚
        simulated_turns = len(resulting_state.history) - len_history_before_simulation
        turn_penalty = simulated_turns * 0.001
        final_score = initial_score - turn_penalty

        print(f"[Scoring] 计算轮次惩罚: {simulated_turns} 轮 * 0.001 = {turn_penalty:.4f}")
    ...
    def nrpa(self, level: int, policy: dict, state: DialogSession) -> DialogSession:
        ...
        for i in range(self.nrpa_iterations):
            ...
            print(f"[NRPA Lvl {level}] 迭代 {i+1}, 候选得分: {candidate_score:.6f}, 状态轮次: {len(candidate_sim_state.history)}")

            if candidate_score > best_score:
                previous_best = "-inf" if best_score == float("-inf") else f"{best_score:.6f}"
                print(f"[NRPA Lvl {level}] **** 新的最佳路径! 得分: {candidate_score:.6f} (优于之前的 {previous_best}) ****")
