...
class NRPAPlanner:
    ...
    def score(self, resulting_state: DialogSession, len_history_before_simulation: int):
        ...
        # 检查是否用户同意捐款
        is_donated = any(turn[1] == PersuasionGame.U_Donate for turn in resulting_state.history if turn[0] == PersuasionGame.USR)
        
        if is_donated:
            print("[Scoring] 检测到 'Donate' 状态，设定基础分为 1.0")
            initial_score = 1.0
        else:
            print("[Scoring] 未检测到 'Donate' 状态，设定基础分为 0.0")
            initial_score = 0.0
