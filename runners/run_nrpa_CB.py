...
def extract_deal_price_with_llm(context, state, backbone_model=None):
    ...
    messages = [
        ...
        {"role": "user", "content": f"Please decide whether the Buyer and the Seller have reached a deal at the end of the conversation. If they have reached a deal, please extract the deal price. You can only reply with one of the following formats:\n- 'They have reached a deal at $[price].' (if deal is reached)\n- 'They have not reached a deal.' (if no deal)\n\nThe following is the conversation:\n{context}\n\nQuestion: Have they reached a deal? Answer:"}
    ]
...
def main(cmd_args):
    ...
    for did in dialog_keys_to_process:
        ...
        history = [(sys_role, CBGame.S_Inquire, "Hi, how much is the %s?" % item_name),
                   (usr_role, CBGame.U_No_deal, "Hi, this is a good %s and its price is %s." % (item_name, seller_price))]
        ...
        if final_state and len(final_state.history) > len(initial_state.history):
            ...
            if is_solved:
                # 首先尝试使用LLM提取价格
                deal_price, llm_deal_confirmed = extract_deal_price_with_llm(current_context, final_state, backbone_model)
                ...
                print(f"📊 SL计算: 买方价格={initial_state.buyer_price}, 卖方价格={initial_state.seller_price}, 交易价格={deal_price} ({price_source})")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str,
                        default=r"D:\GDPZero-master\outputs\gpt-3.5-turbo_nrpa_CB_test_sl_1.pkl",
